\chapter{Coordinator's RESTful API}\label{c:coordinator-api}

\section[/smpc/histogram/numerical POST request]{\protect\icon{figures/post1.pdf}}\label{s:post1}
  ...
  % For instance, for the CVI dataset (see section \ref{c:datasets}) a user specifies that he/she wants to compute a histogram with $\beta = 4$ for term ``Patient Age''.
...
\section[/smpc/histogram/categorical POST request]{\protect\icon{figures/post2.pdf}}\label{s:post2}
...
\section[/smpc/decisionTree POST request]{\protect\icon{figures/post3.pdf}}\label{s:post3}
...
\section[/smpc/queue GET request]{\protect\icon{figures/get1.pdf}}\label{s:get1}
