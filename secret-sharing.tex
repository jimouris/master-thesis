...
\fixme{
A number of systems have implemented various forms of MPC with secret sharing schemes.
...
Another popular system that implements MPC using additive secret shares is the Sharemind Secure Computing Platform.


\subsubsection{Algorithm}\label{sss:additive-algorithm}
\fixme{
Bellow we present a simple additive secret sharing scheme described in \cite{bogdanov2007securely}.
All values in the database and stack are shared among all parties $P_1, \dots, P_n$ by using an additive secret sharing over $\mathbb{Z}_{2^{32}}$.
That is, each party $P_i$ has a local share $S_i$ of a secret value $S$ such that
$S_1 + S_2 + \dots + S_n \equiv S \pmod{2^{32}}$
and any $N − 1$ element subset ${S_{i_1}, \dots, S_{i_{N−1}}}$ is uniformly distributed.
}
...
\fixme{Additive schemes maintain security against a passive and active adversary with unbounded computational power.
Some protocols require a setup phase, which may only be secure against a computationally bounded adversary.
}
...
% From benaloh's paper, needs refinement
\fixme{
Let us suppose we have a secret $S$ that can be computed from $T$ sub-secrets $\{S_i\}^T_{i=1}$ using a functions $f$ as follows $S = f(S_1, \dots, S_T)$.
Let $\oplus$ and $\otimes$ be binary functions on elements of the secret domain $S$ and of the share domain $T$, respectively.
We say that a $(k, N)$ threshold scheme has the $(\oplus, \otimes)$-homomorphism property (or is $(\oplus, \otimes)$-homomorphic) if for all $I = \{i_1, \dots, i_N\}$ and $F_I: T^k \rightarrow S$, whenever
}

\hfil $S = F_I(S_{i_1}, \dots, S_{i_k})$
