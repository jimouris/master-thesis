...
The number of the input parties -- data providers -- as well as the result parties -- analysts/users -- is not restricted, however the computing parties are restricted be the SMPC protocol that is used; for instance in the aforementioned ``Millionaire problem" (\textit{Source Code} \ref{l:millionaires}) is used the \textit{pd\_shared3p}, which utilizes three nodes.
