...
Using this SIMD (Single Input Multiple Data) the protocol performs all operations in parallel by packaging the values in a single communication message.
...
\begin{minted}[linenos,tabsize=2,breaklines,breaksymbolleft=,fontsize=\footnotesize]{c++}
  void main(){
    uint64 N = 5;
    pd_shared3p uint64 a(N) = {1, 2, 3, 4, 5};
    pd_shared3p uint64 b(N) = {6, 7, 8, 9, 10};
    pd_shared3p uint64 c(N);
