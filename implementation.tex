...
In the landing page, except from reading and understanding useful information about the end\hyp to\hyp end infrastructure, the operator can choose between two options: secure data aggregation and secure data classification, as depicted bellow\footnote{In appendix \fixme{add correct ref} \ref{dasasd} we have include the whole landing page screen\hyp shot, as well as many other components of the UI.}.
...
Consecutively, the user can select the type of the dataset, categorical or numerical, as in figure \ref{f:mesh-cvi}\footnote{The option for selecting dataset for secure classification is identical.}.
...
\textbf{Secure Aggregation for CVI:} After selecting the secure aggregation for the CVI (numerical) dataset, the researcher has to choose attributes, filters and data\hyp providers for the privacy computation.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/cvi_histogram.png}
...
\textbf{Secure Aggregation for MeSH:} After selecting the secure aggregation for the MeSH (categorical) dataset, the researcher has to choose attributes, filters and data\hyp providers for the privacy computation.
For instance, in figure \ref{f:mesh-histogram} the user has specified a 2 dimensional histogram for attributes ``Age Groups [M01.060]'' and ``Population Groups [M01.686]'', filtering the ``Diseases [C]'' attribute in ``Virus Diseases [C02]'' or ``Parasitic Diseases [C03]''.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/mesh_histogram.png}
...
\textbf{Secure Classification for CVI:} The UI for secure classification for CVI is similar to the secure aggregation for CVI, however has two extra entries, one for the \emph{class\hyp attribute} and one for the classification algorithm.

\textbf{Secure Classification for MeSH:} Finally, the UI for secure classification is similar to the secure aggregation for MeSH, having two extra entries as well.


More screenshots are included in appendix \fixme{add correct ref} \ref{dasasd}.
