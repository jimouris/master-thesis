  ...
  \For{$attribute \in attributes$}
    \If {$attribute \in categoricalAttributes$}\Comment{{\small If attribute is categorical}}
      \State $\red{splitted} \gets \{\}$
        ...
        \State $\red{maxGain} \gets \red{gt} \hmul \red{gain} \hplus (\enc{1} \hminus \red{gt}) \hmul \red{maxGain}$ \Comment{{\small Obliviously choose  \par\hfill maximum gain}}
    ...
    \Else \Comment{{\small If attribute is numerical}}
      ...
      \For{$i \in \{0, \dots, \textsc{Length}(\red{examples})-2\}$}\Comment{{\small Check all examples, two at a time}}
          ...
          \State $\red{lt} \gets \red{examples}[attribute] \hlte \red{threshold}$ \Comment{{\small $\texttt{SIMD};\red{lt}$ vector gets $\enc{1}$ at positions \par\hfill where the equality holds, $\enc{0}$ otherwise}}
