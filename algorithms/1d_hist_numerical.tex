...
\Procedure{1DimHistogramNumerical}{${\color{darkred}{array}}[N], cells, {\color{darkred}{min}}, {\color{darkred}{max}}$}
    ...
    \For{$i \in \{0, \dots, cells-1\}$}
        \State ${\color{darkred}{eq}} \gets ({\color{darkred}{cellMap}} \heq i) \hplus (({\color{darkred}{cellMap}} \heq cells) \hmul (i == cells - 1)) $\Comment{{\small \texttt{SIMD}; similar to \par\hfill line 3 alg. \ref{a:1d-histogram-categorical}, grouping elements to corresponding cell indexes}}
