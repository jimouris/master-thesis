...
\Procedure{ConstraintMask}{${\color{darkred}{array}}[N][M], operation, constraintAttributes[C], $\par\hfill$ constraintOperators[C], constraintValues[C]$}
    ...
    \For{$c \in \{0, \dots, C-1\}$}\Comment{{\small $C$ is the number of the different constraints}}
        ...
        \If {$constraintOperators = GreaterThan$}
            \State ${\color{darkred}{eq}} \gets {\color{darkred}{constraintAttribute}} \hgt constraintValue$ \Comment{{\small ${\texttt{SIMD}; \color{darkred}{eq}}$ vector gets $\enc{1}$ at \par\hfill positions where the constraint holds, $\enc{0}$ otherwise}}
        ...
        \If {$operation = AND$}
            \State ${\color{darkred}{constraintMask}} \gets {\color{darkred}{constraintMask}} \hmul {\color{darkred}{eq}}$ \Comment{{\small Applying the boolean operator \par\hfill between the constraints. {\small ${\texttt{SIMD}}$}}}
        \ElsIf {$operation = OR$}
