...
On each iteration, ID3 iterates through every unused attribute $T$ of the dataset and calculates the entropy $H_C(T)$ (equation \ref{eq:entropy}) of that attribute.
...
Finally, the set $T$ is then split by the selected attribute $A$ that has the maximum gain -- or equivalently minimum $H_C(T \mid A)$ -- to produce subsets of the data.
