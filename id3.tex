...
As mentioned in section \ref{s:secrec} the $Declassify$ operator publishes a private value.
We should now reason about the selection of public and private data in the described algorithms and also about the usage of the $Declassify$ operator.
...
That is why, everything that the algorithm returns in any of the three cases should be considered as public data (see $Declassify$ operator in lines 9, 11 and public $branches$ variable in line 14).
% Also, in the first termination case of the algorithm, we use the $Declassify$ operator to determine if all examples of the dataset are of the same class.
