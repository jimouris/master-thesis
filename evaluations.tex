...
Figure \ref{chart:id3-patients} portrays how the ID3 algorithm scales as the dataset size grows, for a constant number (3) of classification attributes (``Heart rate'', ``Height (cm)'' and ``Patient Age''), as well as figure \ref{chart:c45-patients} does for C4.5 algorithm\footnote{For many classification attributes C4.5 has an unbearable runtime overhead, and for that reason we do not provide a corresponding chart as in the ID3 algorithm}.
As one can notice, the two algorithms scale similarly with the latter being considerably more time consuming. 
...
\begin{axis}[
  ...
  ylabel={Time (s)},
  xtick = {50, 100, 200, 400, 800, 1600},
  ytick = {500, 600, 800, 1200, 1600, 2400, 3200, 4000, 5800},
  ymax=6000,
  grid style=dashed,
  y tick label style={/pgf/number format/.cd,%
        set thousands separator={}},
...
\addplot
  coordinates {(50, 847)(100, 1066)(200, 1369)(400, 1900)(800, 3162)(1600, 5642)};
\end{axis}
\end{tikzpicture}
